...
const createExperience = asyncHandeler(async (req, res) => {
  ...
  if (!title || !company || !period || !description) {
    throw new ApiError(
      400,
      "Title, company, period, and description are required",
    );
  }
  ...
  return res
    .status(201)
    .json(new ApiResponse(201, experience, "Experience created"));
});
...
const updateExperience = asyncHandeler(async (req, res) => {
  ...
  return res
    .status(200)
    .json(new ApiResponse(200, experience, "Experience updated"));
});
