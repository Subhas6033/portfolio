...
// ── Nav links config ───────────────────────────────────
// isHash: true  → scroll section on homepage (/#section)
// isHash: false → separate route (/projects, /contact)
const LINKS = [
...
function Header() {
  ...
  useEffect(() => {
    if (!isHomePage) { setScrollActive(''); return }
  ...
  return (
    <header className={`fixed top-0 left-0 right-0 z-50 transition-all duration-500 ${
      ...
      <div className="max-w-7xl mx-auto px-6 h-16 flex items-center justify-between">
        ...
        {/* Logo */}
        <a href="/" onClick={(e) => handleClick(e, LINKS[0])} className="flex items-center gap-2.5 group shrink-0">
          ...
          {LINKS.map((l) => (
            <a
              ...
              className={`relative z-10 px-4 py-1.5 rounded-full text-xs font-semibold tracking-wide transition-colors duration-200 select-none ${
                active === l.id ? 'text-zinc-950' : 'text-zinc-400 hover:text-white'
              }`}
            >
              {l.label}
            </a>
        ...
        <div className="hidden md:flex items-center gap-3">
          <a
            href="/contact"
            onClick={(e) => handleClick(e, LINKS.find(l => l.id === 'contact'))}
        ...
        <div className="md:hidden bg-zinc-950/98 backdrop-blur-xl border-t border-zinc-800/60 px-6 pt-4 pb-6">
          <div className="flex flex-col gap-1 mb-5">
            {LINKS.map((l) => (
              ...
              >
                <span className={`w-1.5 h-1.5 rounded-full shrink-0 transition-colors duration-200 ${active === l.id ? 'bg-lime-400' : 'bg-zinc-700'}`} />
                {l.label}
              </a>
            ))}
          </div>
