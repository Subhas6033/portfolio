...
import { EducationFileUploadButton } from "../../utils/uploadthing";
...
const AdminEducation = () => {
  ...
  const createMutation = useMutation({
    mutationFn: (data) => api.post("/api/education", data),
    onSuccess: () => {
  ...
  const updateMutation = useMutation({
    mutationFn: ({ id, data }) => api.put(`/api/education/${id}`, data),
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ["education"] });
      resetForm();
    },
  ...
  const deleteMutation = useMutation({
    mutationFn: (id) => api.delete(`/api/education/${id}`),
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ["education"] });
    },
  });
  ...
  const handleEdit = (edu) => {
    setFormData({
      ...
      resultFileUrl: edu.resultFileUrl || "",
      resultFileName: edu.resultFileName || "",
      resultFileKey: edu.resultFileKey || "",
  ...
  const handleFileUploadComplete = (res) => {
    if (res && res[0]) {
      const fileData = {
        resultFileUrl: res[0].ufsUrl,
        resultFileName: res[0].name,
        resultFileKey: res[0].key,
      };
      ...
      {showForm && (
        ...
        >
          <div className="grid grid-cols-2 gap-4">
            ...
            <div>
              <label className="block text-zinc-400 mb-1">Result URL (Drive/External)</label>
              <input
                type="text"
                value={formData.resultUrl}
                onChange={(e) =>
                  setFormData({ ...formData, resultUrl: e.target.value })
                }
                className="w-full bg-zinc-800 text-white px-3 py-2 rounded"
              />
            </div>
            <div className="col-span-2">
              <label className="block text-zinc-400 mb-1">Or Upload Result File</label>
              <EducationFileUploadButton
                endpoint="educationFileUploader"
                onClientUploadComplete={handleFileUploadComplete}
                onUploadError={(error) => {
                  alert("Upload error: " + error.message);
                }}
              />
              {(formData.resultFileName || uploadedFile) && (
                <p className="text-lime-400 text-sm mt-2">
                  Uploaded: {uploadedFile?.resultFileName || formData.resultFileName}
                </p>
              )}
            </div>
          </div>
